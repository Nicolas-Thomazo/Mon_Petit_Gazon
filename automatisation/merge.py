...
def _clean_col_externes(df:pd.DataFrame)->pd.DataFrame:
    """
    ...
    return df
def _clean_data_externe(df_externe:pd.DataFrame,year)->pd.DataFrame:
    ...
    df_externe = _clean_col_externes(df_externe)
    df_externe = df_externe[(df_externe[SAISON]==year-1) | (df_externe[SAISON]==year)]
    return df_externe

# --------------MPG by season--------------

def _change_type_columns(df:pd.DataFrame)->pd.DataFrame:
    ...
    return df
def _compute_notes_previous_season(df):
    """
    ...
    for i in range(df.shape[0]):
        ...
        if (current_nb_games > 0) and (previous_nb_games>0):
            ...
            df.loc[i,[f"NOTE"]] = 0
    return df
def _compute_previous_season_statistics(df:pd.DataFrame)->pd.DataFrame:
    df_previous = df.copy()
    df_previous[f"NB_MATCH"]=df_previous.loc[:,"Nb match 1 an"] - df["Nb match"]
    df_previous[f"BUTS"]=df_previous.loc[:,"Buts 1 an"] - df["But"]
    df_previous = _compute_notes_previous_season(df_previous)
    df_previous[f"TEMPS"] = df_previous["Tps 1 an"] - df["Temps"]
    return df_previous
def _clean_regroup_mpg_data(df_mpg:pd.DataFrame,year,columns)->pd.DataFrame:
    #df_mpg_clean = change_type_columns(df_mpg)
    df_mpg_clean=df_mpg
    df_mpg_previous = _compute_previous_season_statistics(df_mpg_clean)
...
def _clean_names(name:str)->str:
    ...
    name = name.lower()
    return name
def _normalize_names(players_names:pd.Series)->pd.Series:
    """
    ...
    :rtype: pd.Series
    """
    players_names = players_names.apply(lambda x : _clean_names(x))
...
def _compare_strings(str1:str,str2:str)->float:
    ...
    prop = c/len(str1)
    return prop
def _compute_similarity(player_name:str,df:pd.DataFrame,nb_goals:int):
    """
    ...
    for name in df[JOUEUR]:
        similarity = _compare_strings(name,player_name)
...
def _merge_data(df1:pd.DataFrame,df2:pd.DataFrame)->pd.DataFrame:
    ...
    for saison in df1[SAISON].unique():
        ...
        for player in df1_saison[JOUEUR]:
            ...
            similarity_name,score = _compute_similarity(player,df2[df2[SAISON]==saison],nb_goals)
            if score >0.95:
...
def merge_mpg_externe(PATH:str,first_year:int,last_year:int,file_name:str):
    PATH_EXTERNES_DATA = os.path.join(PATH,f"data_ligue1_players_{first_year}_{last_year+1}.csv")
    PATH_MPG_DATA = os.path.join(PATH,"mpg_stats.xlsx")

    df_externe=pd.read_csv(PATH_EXTERNES_DATA,index_col=0)
    df_mpg=pd.read_excel(PATH_MPG_DATA,header=2,engine="openpyxl")
    columns = [JOUEUR,POSTE,COTE,ENCHERE_MOY,CLUB,NOTE,NB_MATCH,BUTS,TEMPS]

    #Externe data
    df_externe=_clean_data_externe(df_externe,last_year)

    #MPG
    df_mpg=_clean_regroup_mpg_data(df_mpg,last_year,columns)

    #Merge
    df_externe.loc[:,JOUEUR] = _normalize_names(df_externe.loc[:,JOUEUR])
    df_mpg.loc[:,JOUEUR] = _normalize_names(df_mpg.loc[:,JOUEUR])

    df_mpg.loc[:,ASSISTS] = 0
    df_mpg.loc[:,EXPECTED_GOALS] = 0
    df_mpg.loc[:,EXPECTED_ASSISTS] = 0
    df_merge = _merge_data(df_externe,df_mpg)

    path_data = os.path.join(PATH,file_name)
    df_merge.to_csv(path_data)
